...
- (void)testSelectorFromURLAndParams {
    ...
    expect = @{@"p1":@"1",@"p2":@"2"};
    XCTAssertEqual(selector, @selector(aaa:bbb:), @"selector from URL");
    XCTAssertEqualObjects(params, expect, @"params from URL");
    ...
    selector = [self.adapter selectorFromURL:url params:&params];
    expect = @{@"p1":@"1"};
    XCTAssertEqual(selector, @selector(bbb:), @"selector from URL");
    XCTAssertEqualObjects(params, expect, @"params from URL");
    ...
    selector = [self.adapter selectorFromURL:url params:&params];
    expect = @{};
    XCTAssertEqual(selector, @selector(ccc), @"selector from URL");
    XCTAssertEqualObjects(params, expect, @"params from URL");
    ...
    selector = [self.adapter selectorFromURL:url params:&params];
    expect = @{};
    XCTAssertEqual(selector, @selector(crash:), @"selector from URL");
    XCTAssertEqualObjects(params, expect, @"params from URL");
}
...
- (void)testWebViewDelegateLinkClicked1 {
    ...
    XCTAssertTrue(self.delegateAction1Called, @"delegateAction1Called");
}
...
- (void)testWebViewDelegateLinkClicked2 {
    ...
    [self.adapter webView:nil shouldStartLoadWithRequest:req navigationType:UIWebViewNavigationTypeLinkClicked];
    XCTAssertTrue(self.delegateAction2Called, @"delegateAction2Called");
}
...
- (void)testWebViewDelegateLinkClicked3 {
    ...
    [self.adapter webView:nil shouldStartLoadWithRequest:req navigationType:UIWebViewNavigationTypeLinkClicked];
    XCTAssertTrue(self.delegateAction3Called, @"delegateAction3Called");
